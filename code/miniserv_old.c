	...
	while (1)
	{
		...
		else if (retval)
		{
			if (FD_ISSET(socketfd, &rfds))
			{
				...
				FD_SET(array_of_client[i].client_fd, &rfds);
				if (taller_fd < client_fd)
					taller_fd = client_fd;
			}
			else
			{
				...
				char	tmp[10];
				ssize_t res = recv(c.client_fd, tmp, 10, 0);
				if (res <= 0)
				{
					char	msg[strlen(msg_exit_c) + get_nbr_char(array_of_client[i].id) - 2];
					size_t j = 0;
					sprintf(msg, msg_exit_c, c.id);
					my_print("HER\n", 2);
					while (j < SOMAXCONN)
					{
						if (array_of_client[j].libre == 0)
							send(array_of_client[j].client_fd, msg, strlen(msg_exit_c) + get_nbr_char(c.id) - 2, 0);
						j++;
					}
